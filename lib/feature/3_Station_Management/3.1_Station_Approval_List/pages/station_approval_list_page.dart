...
import 'package:web_netpool_station_platform_admin/feature/3_Station_Management/3.1_Station_Approval_List/model/station_approval_list/station_approval_list_model.dart';
import 'package:web_netpool_station_platform_admin/feature/3_Station_Management/3.1_Station_Approval_List/pages/station_approval_list_source.dart';
import 'package:web_netpool_station_platform_admin/feature/Common/snackbar/snackbar.dart';

//! Station Approval - DS Station Approval !//

class StationApprovalListPage extends StatefulWidget {
  const StationApprovalListPage({super.key});

  @override
  State<StationApprovalListPage> createState() =>
      _StationApprovalListPageState();
}

class _StationApprovalListPageState extends State<StationApprovalListPage> {
  final StationApprovalListBloc stationApprovalListBloc =
      StationApprovalListBloc();
  // Approval<StationApprovalListModel> StationApprovalList = [];
  ...
  void initState() {
    ...
    _dataSource = StationDataSource(
        context: context, initialData: [], bloc: stationApprovalListBloc);
    _paginatorController = PaginatorController();

    stationApprovalListBloc.add(StationApprovalListInitialEvent());
  }
  ...
  void _sort<T extends Comparable>(
    T? Function(StationApprovalListModel d) getField,
    int columnIndex,
    bool ascending,
  ) {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<StationApprovalListBloc, StationApprovalListState>(
      bloc: stationApprovalListBloc,
      listenWhen: (previous, current) =>
          current is StationApprovalListActionState,
      ...
      builder: (context, state) {
        if (state is StationApprovalListSuccessState) {
          isLoading = false;
          ...
          _dataSource.updateData(
              state.StationApprovalList, _totalRows, pageOffset);
        }
        if (state is StationApprovalListEmptyState) {
          isLoading = false;
          _totalRows = 0;
          _dataSource.updateData([], 0, 0);
        }
        if (state is StationApprovalList_LoadingState) {
              ...
              children: [
                PaginatedDataTable2(
                  ...
                  onPageChanged: (pageIndex) {
                    ...
                    stationApprovalListBloc.add(StationApprovalListLoadEvent(
