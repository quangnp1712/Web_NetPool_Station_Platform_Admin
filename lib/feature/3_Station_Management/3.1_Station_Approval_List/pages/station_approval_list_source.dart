...
class StationDataSource extends DataTableSource {
  List<StationApprovalListModel> _StationApprovalList =
      []; // Sửa: Dữ liệu của trang hiện tại
  final BuildContext context;
  StationApprovalListBloc bloc;
  ...
  StationDataSource({
    ...
    required this.bloc,
    required List<StationApprovalListModel> initialData,
  }) {
    _StationApprovalList = initialData;
  }

  /// Cập nhật dữ liệu (ví dụ: khi BLoC load xong)
  // SỬA: Thêm totalRows và pageOffset
  void updateData(List<StationApprovalListModel> newApproval, int totalRows,
      int pageOffset) {
    _StationApprovalList = newApproval;
  ...
  void sort<T extends Comparable>(
    ...
    _StationApprovalList.sort((a, b) {
      // Sửa: Sắp xếp _StationApprovalList
  ...
  DataRow? getRow(int index) {
    ...
    if (localIndex < 0 || localIndex >= _StationApprovalList.length) {
    ...
    final data = _StationApprovalList[localIndex];
        ...
        DataCell(Row(
          children: [
            IconButton(
              ...
              onPressed: () {
                if (data.stationId != null) {
                  bloc.add(AcceptEvent(stationId: data.stationId!));
                }
              },
              tooltip: "Chấp thuận",
            ),
            IconButton(
              ...
              onPressed: () {
                if (data.stationId != null) {
                  bloc.add(RejectEvent(
                      stationId: data.stationId!, rejectReason: ""));
                }
              },
              tooltip: "Từ chối",
            ),
