...
import 'package:web_netpool_station_platform_admin/feature/4_Account_Admin_Management/4.1_Account_Admin_List/pages/admin_data_source.dart';
    ...
    return BlocConsumer<AdminListBloc, AdminListState>(
      ...
      builder: (context, state) {
        if (state is AdminListSuccessState) {
          ...
          _roleName = state.roleName;

          // Tính toán offset (vị trí bắt đầu) của trang hiện tại
          final pageOffset = (_currentPage - 1) * _rowsPerPage;

          _dataSource.updateData(
              state.adminList, _totalRows, pageOffset, _roleName);
        }
        if (state is AdminListEmptyState) {
          _totalRows = 0;
          _dataSource.updateData([], 0, 0, "");
          statusNames = [];
        }
        isLoading = (state is AdminList_LoadingState && state.isLoading);
        ...
        children: [
          ...
          Padding(
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                adminListBloc.add(ShowAdminCreatePageEvent());
              },
